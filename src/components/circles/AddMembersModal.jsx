    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
      <div className="bg-white rounded-xl shadow-xl max-w-2xl w-full max-h-[80vh] flex flex-col">
        ...
        <div className="p-6 border-b border-gray-200 flex items-center justify-between flex-shrink-0">
          <div>
            <h2 className="text-xl font-bold text-gray-800">{trf('Add Members to {0}', [circle.name])}</h2>
            <p className="text-sm text-gray-600 mt-1">
              {trf('{0} selected • {1} available', [selectedMemberIds.length, availableMembers.length])}
            </p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto p-4">
          {filteredMembers.length === 0 ? (
            <div className="text-center py-12 text-gray-500">
              {searchTerm ? tr('No members match your search') : tr('No available members')}
            </div>
          ) : (
        ...
        <div className="p-6 border-t border-gray-200 flex justify-end gap-3 flex-shrink-0">
          ...
          >
            ...
            {tr('Add')} {selectedMemberIds.length > 0 && `(${selectedMemberIds.length})`}
          </button>
        </div>
      </div>
    </div>
