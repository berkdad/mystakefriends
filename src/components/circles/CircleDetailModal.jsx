      ...
      <div className="bg-white rounded-xl shadow-xl max-w-2xl w-full max-h-[90vh] flex flex-col">
        ...
        <div className={`p-6 border-b flex-shrink-0 ${
          isEditMode
            ...
            : 'bg-gradient-to-r from-primary-50 to-accent-50 border-gray-200'
        }`}>
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3 flex-1">
              ...
              {isEditingName ? (
                <input
                  ...
                  className="flex-1 px-3 py-1.5 border border-primary-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-transparent text-lg font-bold"
                  autoFocus
                />
              ) : (
                <div className="flex items-center gap-2">
            ...
            <button
              ...
              title={isEditMode ? tr('Click to go Live') : tr('Click to Edit mode')}
            >
              {isEditMode ? (
                <>
        ...
        <div className="flex-1 overflow-y-auto p-6">
          {showAddMembers ? (
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <h3 className="font-semibold text-gray-800">{tr('Add Members')}</h3>
                <button
                  onClick={() => {
                    setShowAddMembers(false);
                    setSelectedForAdd([]);
              ...
              <input
                ...
                placeholder={tr('Search available members...')}
              ...
              <div className="space-y-2 max-h-80 overflow-y-auto">
                {filteredAvailableMembers.length === 0 ? (
                  <p className="text-center py-8 text-gray-500">
                    {addSearchTerm ? tr('No members match your search') : tr('No available members')}
                  </p>
                ) : (
              ...
              {selectedForAdd.length > 0 && (
                ...
                >
                  ...
                  {selectedForAdd.length !== 1
                    ? trf('Add {0} Members', [selectedForAdd.length])
                    : trf('Add {0} Member', [selectedForAdd.length])}
                </button>
              )}
            </div>
          ) : (
            ...
            <div className="space-y-4">
              ...
              <button
                ...
                className="w-full flex items-center justify-center gap-2 px-4 py-2 border-2 border-dashed border-gray-300 text-gray-600 rounded-lg hover:border-primary-300 hover:text-primary-600 hover:bg-primary-50 transition-colors"
              >
                <Plus className="w-4 h-4" />
                {tr('Add Members')}
              </button>
              ...
              {members.length === 0 ? (
                <div className="text-center py-12 text-gray-500">
                  ...
                  <p>{tr('No members in this circle')}</p>
                  <p className="text-sm mt-1">{tr('Click "Add Members" to get started')}</p>
                </div>
              ) : (
                <>
                  {/* Captain section */}
        ...
        {!showAddMembers && selectedForRemoval.length > 0 && (
          <div className="p-4 border-t border-gray-200 bg-gray-50 flex-shrink-0">
            ...
            >
              ...
              {selectedForRemoval.length !== 1
                ? trf('Remove {0} Members', [selectedForRemoval.length])
                : trf('Remove {0} Member', [selectedForRemoval.length])}
            </button>
          </div>
        )}
      </div>
