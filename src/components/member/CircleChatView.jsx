...
export default function CircleChatView({ db, storage, stakeId, wardId, circleId, currentMemberId, currentMemberName, circleMembers }) {
  ...
  const handleFileUpload = async (e) => {
    ...
    try {
      ...
      alert(tr('Error uploading file. Please try again.'));
    } finally {
      setUploading(false);
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      alert(tr('Error sending message. Please try again.'));
    } finally {
      setSending(false);
    }
  };
  ...
  const formatTimestamp = (timestamp) => {
    ...
    if (diffMins < 60) return trf('{0}m ago', [diffMins]);
    if (diffHours < 24) return trf('{0}h ago', [diffHours]);
    if (diffDays < 7) return trf('{0}d ago', [diffDays]);
  ...
  return (
    <div className="bg-white rounded-xl shadow-md border border-blue-100 flex flex-col h-[600px]">
      ...
      <div className="p-4 border-b border-gray-200">
        <h2 className="text-xl font-semibold text-gray-800">{tr('Circle Chat')}</h2>
        <p className="text-sm text-gray-600">{trf('{0} members', [circleMembers.length])}</p>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        {messages.length === 0 ? (
          <div className="text-center py-12 text-gray-500">
            <p className="mb-2">{tr('No messages yet')}</p>
            <p className="text-sm">{tr('Start the conversation!')}</p>
          </div>
        ) : (
          messages.map((msg) => {
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-xs text-gray-500">
                            {trf('{0} KB', [(msg.fileSize / 1024).toFixed(1)])}
                          </p>
                        </div>
                        <a
                          href={msg.fileUrl}
